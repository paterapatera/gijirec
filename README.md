...
- マイク＋スピーカー（ループバック）を 16 kHz モノラル PCM にリアルタイム合成（起動後は待機。再生で会議開始。途中の利用者向け停止はなし）
...
- 手書きメモ（左）と AI 転写（右）の二重エディタ（ペイン別独立スクロール、部分ロック、タイムスタンプ維持）
...
2. マイク・システム音声の権限を許可し、会議開始（再生）後に UI が `capturing` になることを確認する。
