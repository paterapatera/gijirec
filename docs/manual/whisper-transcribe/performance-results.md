...
## 転写 ingest ゲイン（音量正規化）手動検証

`PcmIngestConsumer` の `TRANSCRIBE_INGEST_GAIN = 1.25` + `TRANSCRIBE_SOFT_LIMIT = 0.95` 適用後、快適 OS 音量で推論窓 RMS が −18〜−17 dBFS 付近になること。

| 項目 | 値 |
|------|-----|
| **記録日** | 2026-09-08 |
| **実行者** | 開発者（transcribe-volume-normalize 完了検証） |
| **結果** | **合格** — 30 s 窓 RMS 平均 **約 −17.1 dBFS**（要件 −18〜−17 付近） |
| **ログ** | セッション `20260908T235433Z` の `transcribe_window_rms_dbfs` |
| **備考** | ミキサー・Whisper アダプタ前処理は未変更。ingest ゲインのみ |
