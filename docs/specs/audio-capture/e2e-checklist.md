...
設計 [E2E/UI Tests](../design.md) 項目 1–2 の実行手順。Playwright 等のブラウザ E2E フレームワークは未導入。項目 1–2 は React コンポーネントテストでカバーし、実機 Tauri 起動は半自動（手動確認）とする。
...
ライフサイクル結線（7.1）経由で Tauri 起動時に `capturing` へ遷移することを目視確認する。
...
設計 E2E/UI Test 3（要件 **3.2**）および Performance/Load 4（要件 **4.1**）。Zoom / Teams 並走時の相手音声途切れと、ウィンドウ閉鎖後のマイクインジケータ消灯は **実機のみ** で確認する。
