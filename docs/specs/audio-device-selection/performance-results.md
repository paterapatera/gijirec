...
設計 [Performance/Load](design.md) 項目 1、要件 **5.3**（選択変更 → `capturing` 復帰 **< 2 s**）。
計測は `device_selection_restart_duration_ms`（tracing / observability）および統合テストの wall-clock で記録する。数値の捏造は禁止 — テスト実行後の実測値のみ記載する。
...
- [design.md — Performance/Load / Operational Readiness](design.md)
- [device_selection_observability.rs](../../src-tauri/tests/device_selection_observability.rs)（tracing フィールド検証）
