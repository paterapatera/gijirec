...
- **Command 面**: `src/infrastructure/tauri/editorCommands.ts`（save / settings / pick directory）、`src/infrastructure/tauri/audioDeviceCommands.ts`（デバイス一覧・選択）、`src/infrastructure/tauri/transcribeSettingsCommands.ts`（転写バリアント設定）、`src/infrastructure/tauri/captureAudioControlsCommands.ts`（音声制御 get/set）。イベント購読ではなく invoke ラップ
- **音声制御 hook**: `src/presentation/hooks/useCaptureAudioControls.ts`（起動時 `get_capture_audio_controls`・`controls-changed` / `ingest-level` 購読）
