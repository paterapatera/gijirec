...
**Transcribe 固有**: `TranscribeError::to_user_facing()` → `whisper-transcribe://error`。上流キャプチャ `error` は `UPSTREAM_CAPTURE_ERROR` で伝播。未処理 PCM が 1 時間設計上限に達した場合はサイレント drop せず `PCM_RETENTION_LIMIT_EXCEEDED`（`recoverable: true`、新セッション開始を促す）。転写設定の読み込み失敗（欠落・破損 JSON）は FP16 既定で起動継続（ログ＋利用者通知、invoke エラーにしない）。保存失敗は `SETTINGS_PERSIST_FAILED` を invoke エラーとして返す。
...
---
_updated_at: 2026-09-19（PCM 保持上限到達エラーを追記）_
