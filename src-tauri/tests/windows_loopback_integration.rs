...
/// Must match `#[ignore = "..."]` on `integration_mic_and_wasapi_loopback_reach_capturing_on_hardware` in compose/tests.rs.
...
fn integration_test_1_documents_windows_wasapi_loopback_ci_skip_in_source() {
    ...
    assert!(
        ...
        "compose/tests.rs must define mic+loopback hardware integration test"
    );
    assert!(
