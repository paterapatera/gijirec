...
describe("showSaveResult", () => {
  ...
  test("shows message_ja and action_ja via toast.error when save fails", () => {
    ...
    const args = mockToastError.mock.calls[0]! as unknown as [string, { description?: string }];
    const payload = stringifyToastArgs(args);
    const error = result.error!;
    expect(args[0]).toBe(error.message_ja);
    expect(payload).toContain(error.action_ja);
  });
  ...
  test("does not display error code as user-facing toast text", () => {
    ...
    const payload = stringifyToastArgs(
      mockToastError.mock.calls[0]! as unknown as [string, { description?: string }],
    );
    const error = result.error!;
    expect(payload).not.toContain("SAVE_FILE_WRITE_FAILED");
    expect(payload).toContain(error.message_ja);
    expect(payload).toContain(error.action_ja);
  });
